//! 使用 Octocrab API 客户端分析 GitHub 仓库。
...
/// `github.com/owner/repo`、`owner/repo`（可选的 `.git` 后缀）。
///
/// # 参数
            ...
            for file in files {
                if is_source_file(&file.filename) {
                    let entry = file_change_count
                        ...
                        .or_insert((0, 0, 0));
                    entry.0 += 1;
                    // Octocrab 的 DiffEntry 包含 additions 和 deletions 字段
